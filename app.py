...
def place_order():
    try:
        username, phone, location = request.form.get('username'), request.form.get('phone'), request.form.get('location')
        ...
        return redirect(url_for('index', success='true', method=method))
    except Exception as e:
    ...
    try:
        ...
        if file:
            ...
            supabase.table("products").insert({
                ...
                "price": int(request.form.get('price')), "image_url": img_url
            }).execute()
        return redirect(url_for('admin', tab='inventory'))
    except:
        return redirect(url_for('admin', tab='inventory'))
